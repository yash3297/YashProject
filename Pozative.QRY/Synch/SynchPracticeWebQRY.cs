...
{
    public class SynchPracticeWebQRY
    {

        public static string GetPracticeWebAppointmentData = " SELECT a.AptNum AS Appt_EHR_ID,pat.PatNum AS PatNum,pat.PatNum AS Patient_EHR_ID,pat.LName AS Last_Name,pat.FName AS First_Name,pat.MiddleI AS MI, "
                                                          ...
                                                          + " IFNull(o.clinicnum,a.clinicnum) as Clinic_Number "
                                                          + " FROM appointment a "
        ...
        public static string GetPracticeWebAppointmentEhrIDs = " SELECT a.AptNum AS Appt_EHR_ID"
                                                          + " FROM appointment a "
                                                          + " JOIN patient pat ON pat.PatNum = a.PatNum "
                                                          + " LEFT JOIN operatory o ON a.OP = o.OperatoryNum  "
                                                          + " Left JOIN appointmenttype aptype ON aptype.AppointmentTypeNum = a.AppointmentTypeNum "
        ...
        public static string PracticeWebAppointment_Procedures_Data = " select b.aptnum,group_concat(procedurecodes separator ',') as ProcedureCode, group_concat(procdesc separator ',') as ProcedureDesc "
        ...
        public static string GetPracticeWebAppointmentData_15_4 = " SELECT a.AptNum AS Appt_EHR_ID,pat.PatNum AS PatNum,pat.LName AS Last_Name,pat.FName AS First_Name,pat.MiddleI AS MI, "
                                                          + " pat.HmPhone AS Home_Contact, pat.WirelessPhone AS Mobile_Contact,pat.Email AS Email,pat.Address AS Address, "
                                                          + " pat.City AS City,pat.State AS ST,pat.Zip AS Zip,case when a.OP is null then 0 else a.OP end AS Operatory_EHR_ID,o.OpName AS Operatory_Name, "
                                                          + " pro.ProvNum AS Provider_EHR_ID, CONCAT(pro.LName, ' ' , pro.FName) AS Provider_Name , "
                                                          + " a.AppointmentTypeNum AS ApptType_EHR_ID, aptype.AppointmentTypeName AS ApptType , "
                                                          ...
                                                          + " WHERE  a.AptDateTime > @ToDate ; "; // AND a.OP is not null
        //+ " WHERE (a.DateTStamp > @ToDate OR a.AptDateTime > @ToDate) ; "; // AND a.OP is not null
        public static string GetPracticeWebAppointmentEhrIDs_15_4 = " SELECT a.AptNum AS Appt_EHR_ID"
                                                          + " FROM appointment a "
                                                          + " JOIN patient pat ON pat.PatNum = a.PatNum "
                                                          + " LEFT JOIN operatory o ON a.OP = o.OperatoryNum  "
                                                          + " Left JOIN appointmenttype aptype ON aptype.AppointmentTypeNum = a.AppointmentTypeNum "
        ...
        public static string GetOpenDentalPatientBalanceData = "SELECT sum(PP.Payamt) as Balance, PP.PatNum as Patient_ehr_ID from payment PP  inner join appointment AP on AP.patnum = PP.patnum where AP.aptstatus = 2 Group by PP.paynum";

        public static string GetOpenDentalPatientBalanceData_Clinic = "SELECT sum(PP.Payamt) as Balance, PP.PatNum as Patient_ehr_ID from payment PP  inner join appointment AP on AP.patnum = PP.patnum where AP.aptstatus = 2 Group by PP.paynum where AP.clinicnum = @clinicnum";


        public static string GetPracticeWebPatientStatusNew_Existing = "select patnum as Patient_EHR_Id from patient where PatNum <> ''  AND LName <> '' and (Not ((FName = '' OR LName = '') and patstatus = 4))  and patnum not in (select p.patnum as Patient_EHR_Id from patient p INNER join appointment a on a.patnum = p.patnum where  a.aptdatetime < curdate() and a.AptStatus = 2) "; //and (DateFirstVisit = '' OR DateFirstVisit = '0001-01-01' OR DateFirstVisit < curdate())

        public static string GetPracticeWebPatientStatusNew_Existing_Clinic = "select patnum as Patient_EHR_Id from patient where PatNum <> ''  AND LName <> '' and (Not ((FName = '' OR LName = '') and patstatus = 4))  and patnum not in (select p.patnum as Patient_EHR_Id from patient p INNER join appointment a on a.patnum = p.patnum where  a.aptdatetime < curdate() and a.AptStatus = 2) and (clinicnum = @Clinic_Number) "; // and (DateFirstVisit = '' OR DateFirstVisit = '0001-01-01' OR DateFirstVisit < curdate())

        public static string GetPracticeWebDeletedAppointmentData = " SELECT ObjectNum As Appt_EHR_ID, DateTStamp AS EHR_Entry_DateTime "
                                                           + " FROM deletedobject WHERE ObjectType = 0 AND DateTStamp > @ToDate ; ";

        public static string GetPracticeWebDeletedAppointmentData_Clinic_Number = " SELECT d.ObjectNum As Appt_EHR_ID, d.DateTStamp AS EHR_Entry_DateTime ,IFNull(o.clinicnum,a.clinicnum) as Clinic_Number "
        ...
        public static string GetPracticeWebDeletedAppointmentData17_2Plus = " SELECT h.AptNum AS Appt_EHR_ID,IFNull(o.clinicnum,h.clinicnum) as Clinic_Number FROM histappointment h LEFT JOIN operatory o ON h.OP = o.OperatoryNum  where HistApptAction = 4 AND HistDateTStamp > @ToDate "
                                                          + " union "
                                                          + "   select DISTINCT aptnum as Appt_EHR_ID, clinicnum as Clinic_Number "
                                                          + "   from appointment where aptstatus = 3 and op = 0 and AptDateTime >= @ToDate ; ";

        //public static string GetPracticeWebPatientInsuranceData = "SELECT distinct p.PatNum as Patient_EHR_ID,ip.carrierNum as Primary_Insurance,c.CarrierName as Primary_Insurance_CompanyName ,c.Phone as Prim_Ins_Company_Phonenumber,i.SubscriberID, pat.clinicnum as Clinic_Number, GroupNum as groupid "
        ...
        //                                                 + " p.InsSubNum = i.InsSubNum join insplan ip on ip.PlanNum = i.PlanNum "
        //                                                 + " join patient pat on pat.PatNum = p.PatNum "
        //                                                 + " join carrier c on c.carrierNum = ip.carrierNum Group By p.PatNum; ";
        ...
        public static string GetPracticeWebOperatoryEventData = " SELECT IFNULL(OP.ScheduleOpNum,S.ScheduleNum) AS ScheduleNum ,S.SchedDate,S.StartTime,S.StopTime,S.Note,S.DateTStamp, IFNULL(OP.OperatoryNum,0) AS OperatoryNum, IFNULL(o.clinicnum,0) as Clinic_Number "
        ...
        public static string GetPracticeWebOperatoryAllEventData = " SELECT IFNULL(OP.ScheduleOpNum,S.ScheduleNum) AS ScheduleNum ,S.SchedDate,S.StartTime,S.StopTime,S.Note,S.DateTStamp, IFNULL(OP.OperatoryNum,0) AS OperatoryNum,IFNULL(o.clinicnum,0) as Clinic_Number "
                                                          + " FROM schedule S JOIN scheduleop OP ON S.ScheduleNum = OP.ScheduleNum left join operatory o on o.OperatoryNum = OP.OperatoryNum "
                                                          + " WHERE  S.SchedDate > @ToDate and S.SchedType = 2;";

        //public static string GetPracticeWebHoliday = " Select ScheduleNum, SchedDate, StartTime, StopTime,SchedType,ProvNum,BlockoutType,Note,Status,DateTStamp,clinicnum  as Clinic_Number from schedule "
        //                                          + " WHERE SchedType = 0 AND DateTStamp > @ToDate ; ";

        public static string GetPracticeWebHoliday = " Select CAST(ScheduleNum AS CHAR) as ScheduleNum, SchedDate, StartTime, StopTime,SchedType,ProvNum,BlockoutType,Note,Status,DateTStamp,clinicnum as Clinic_Number from schedule "
                                                  + " WHERE SchedType = 0 AND DateTStamp > @ToDate ; ";

        public static string GetPracticeWebHoliday_15_4 = " Select CAST(SD.ScheduleNum AS CHAR) as ScheduleNum, SD.SchedDate, SD.StartTime, SD.StopTime,SD.SchedType,SD.ProvNum,SD.BlockoutType,SD.Note,SD.Status,SD.DateTStamp,IfNull(o.ClinicNum,0) as Clinic_Number "
        ...
        //public static string GetPracticeWebProviderData = " SELECT p.provnum AS Provider_EHR_ID,p.lname AS Last_Name,p.fname AS First_Name, p.mi, "
        ...
        public static string GetPracticeWebProviderData = " SELECT p.provnum AS Provider_EHR_ID,p.lname AS Last_Name,p.fname AS First_Name, p.mi, "
                                                       + " d.itemname AS provider_speciality, (case when p.IsHidden = 0 then 1 else 0 end) as is_active, IfNULL(PCL.ClinicNum,0) as Clinic_Number "
                                                       + " FROM provider p JOIN definition d ON d.DefNum = p.Specialty LEFT JOIN providerclinic PCL ON PCL.ProvNum = p.provnum ; ";

        public static string GetPracticeWebProviderData_link = " SELECT p.provnum AS Provider_EHR_ID, p.lname AS Last_Name, p.fname AS First_Name, p.mi, "
                                                  + " d.itemname AS provider_speciality, (case when p.IsHidden = 0 then 1 else 0 end) as is_active, IfNULL(PCL.ClinicNum,0) as Clinic_Number "
                                                  + " FROM provider p JOIN definition d ON d.DefNum = p.Specialty LEFT JOIN providercliniclink PCL ON PCL.ProvNum = p.provnum ; ";

        public static string GetPracticeWebProviderData_15_4 = " SELECT p.provnum AS Provider_EHR_ID, p.lname AS Last_Name, p.fname AS First_Name, p.mi, "
                                                            + " d.itemname AS provider_speciality, (case when p.IsHidden = 0 then 1 else 0 end) as is_active, 0 as Clinic_Number "
                                                            + " FROM provider p JOIN definition d ON d.DefNum = p.Specialty ; ";

        //public static string GetPracticeWebProviderHoursData = " SELECT sd.scheduleNum,IFNULL(sdo.ScheduleOpNum,0), "
        ...
        public static string GetPracticeWebProviderHoursData_15_4 = " SELECT sd.scheduleNum,IFNULL(sdo.ScheduleOpNum,0), "
        ...
        public static string GetPracticeWebOperatoryData = "SELECT OperatoryNum AS Operatory_EHR_ID,OpName AS Operatory_Name,clinicnum as Clinic_Number,ItemOrder AS OperatoryOrder,IsHidden as Is_Deleted FROM operatory o;";

        public static string GetPracticeWebDeletedOperatoryData = "SELECT OperatoryNum AS Operatory_EHR_ID,OpName AS Operatory_Name,clinicnum as Clinic_Number FROM operatory o where IsHidden = 1 ;";

        public static string GetPracticeWebApptTypeData = "SELECT AppointmentTypeNum AS ApptType_EHR_ID, AppointmentTypeName AS Type_Name,'' as Clinic_Number FROM appointmenttype a;";
        ...
        public static string Is_Update_Status_EHR_Appointment_Live_To_PracticeWeb = "SELECT AptStatus, Confirmed,IFNull(o.clinicnum,a.clinicnum) as Clinic_Number  FROM appointment LEFT JOIN operatory o ON a.OP = o.OperatoryNum  Where AptNum  = @Appt_EHR_ID;";

        //public static string GetPracticeWebPatientData = " SELECT p.PatNum AS Patient_EHR_ID,FName AS First_name,LName AS Last_name,MiddleI AS Middle_Name,Title AS Salutation, "
        ...
        public static string GetPracticeWebInsertPatientData = "Select p.PatNum as Patient_EHR_ID,p.clinicnum as Clinic_Number from Patient p where p.PatNum <> '' AND p.lname <> '' and ( Not( ( p.fname = '' OR p.lname = '' ) and p.patstatus = 4 ) ) and (p.clinicnum = @Clinic_Number);";

        public static string GetPracticeWebPatientData_Clinic = "SELECT DISTINCT '0' as 'Patient_LocalDB_ID', p.PatNum AS Patient_EHR_ID, '0' as 'Patient_Web_ID', p.fname AS First_name, p.lname AS Last_name, p.MiddleI AS Middle_Name, p.Salutation," +
                                                                ...
                                                                "or Cast(p.TxtMsgOk As Char) = '1' then 'Y' else 'N' end ) as ReceiveSMS, p.email as ReceiveEmail, NextVisit_Date, " +
                                                                "DD.DueDate as due_date, IFNULL(AC.TotalBenafit, 0) AS remaining_benefit, CP.CollectPayment AS collect_payment, Now() as Last_Sync_Date," +
        ...
        public static string GetPracticeWebAppointmentsPatientData = " SELECT DISTINCT p.PatNum AS Patient_EHR_ID,p.FName AS First_name,p.lname AS Last_name,p.MiddleI AS Middle_Name,p.Salutation, "
                                                     + "   p.Preferred As preferred_name,  Cast(p.Gender As char(7)) AS Sex, Cast(p.Position As char(8)) AS MaritalStatus,"
        ...
        public static string GetPracticeWebAppointmentsPatientData_Clinic = " SELECT DISTINCT p.PatNum AS Patient_EHR_ID,p.FName AS First_name,p.lname AS Last_name,p.MiddleI AS Middle_Name,p.Salutation, "
                                                   + "   p.Preferred As preferred_name,  Cast(p.Gender As char(7)) AS Sex, Cast(p.Position As char(8)) AS MaritalStatus,"
                                                   + "   p.Birthdate AS Birth_Date,p.Email AS Email,p.WirelessPhone AS Mobile,p.HmPhone AS Home_Phone, "
                                                   + "   p.WkPhone AS Work_Phone,p.Address AS Address1,p.Address2 AS Address2,p.City AS City,p.State AS State,p.Zip AS Zipcode,p.EstBalance AS CurrentBal, "
                                                   + "   p.Bal_0_30 AS ThirtyDay,p.Bal_31_60 AS SixtyDay,p.Bal_61_90 AS NinetyDay,p.BalOver90 AS Over90,p.DateFirstVisit AS FirstVisit_Date, "
                                                  ...
                                                  + " p.SSN as ssn, p.SchoolName as school, '' as driverlicense,emp.EmpName AS employer, "
                                                   ...
                                                   + "   PG.spouseId,PG.Spouse_First_Name,PG.Spouse_Last_Name,'' as Sec_Ins_Company_Phonenumber, '' AS Prim_Ins_Company_Phonenumber , '' AS groupid "
                                                  + " FROM patient p "
                                                  + " Inner Join appointment a ON p.PatNum = a.PatNum left join employer as emp on p.EmployerNum = emp.EmployerNum  "
                                                  + " left join patientnote as pn on p.Patnum=pn.Patnum "
                                                     + "  left join patient as resp on p.Patnum=resp.responsparty and resp.responsparty > 0"
                                                     + "  left join (select patnumchild, max(patnumguardian) as spouseId,FName as Spouse_First_Name,LName as Spouse_Last_Name "
                                                     + "  from patient pa,guardian g where pa.patnum=g.patnumguardian and relationship =18 group by patnumchild ) PG on p.patnum = PG.patnumchild "
        ...
        public static string GetPracticeWebApptPatientdue_date = "SELECT r.recallNum, rt.RecallTypeNum, r.DateDue AS due_date, r.patnum AS patient_id, rt.description AS recall_type,p.clinicnum As Clinic_Number "
                                                  + " FROM recall r "
                                                  + " Left Join patient p on p.PatNum = r.PatNum "
                                                  + " JOIN recalltype rt on r.RecallTypeNum = rt.RecallTypeNum inner join appointment ap on ap.patnum = p.patnum where ap.aptdatetime > @ToDate and p.clinicnum = @clinicnum;";

        #endregion

        public static string GetPracticeWebPatientDocData = " SELECT p.PatNum AS Patient_EHR_ID,FName AS First_name,LName AS Last_name,ImageFolder,clinicnum as Clinic_Number "
                                                     + " FROM patient p ";

        public static string GetPracticeWebPatientImagesData = @" 
select
...
";
        ...
        public static string GetPatientWisePendingAmount = " SELECT AA.PatNum,AA.clinicnum as Clinic_Number, IFNULL(AB.PaymentAmount,0) AS Current_Payment, ( AA.TotalBill - IFNULL(AB.PaymentAmount,0) ) AS Remaining_Benefit FROM "
                                                         ...
                                                         + " LEFT JOIN ( SELECT SUM(PayAmt) AS PaymentAmount, PatNum,clinicnum FROM payment  GROUP BY PatNum,clinicnum ) AS AB ON AA.PatNum = AB.PatNum And AA.clinicnum = AB.clinicnum ";

        public static string GetPracticeWebPatientID_NameData = "  SELECT p.PatNum AS Patient_EHR_ID,FName AS FirstName,LName AS LastName, CONCAT(FName, ' ', LName) AS Patient_Name, REPLACE(REPLACE(REPLACE(REPLACE(WirelessPhone,'(',''),')',''),'-',''),' ','') AS Mobile, REPLACE(REPLACE(REPLACE(REPLACE(HmPhone,'(',''),')',''),'-',''),' ','') AS Home_Phone, REPLACE(REPLACE(REPLACE(REPLACE(WkPhone,'(',''),')',''),'-',''),' ','') AS Work_Phone,Birthdate AS birth_date, Guarantor,clinicnum  as Clinic_Number,p.Email FROM patient p;";

        public static string GetPracticeWebIdelProvider = " SELECT ProvNum FROM provider where ishidden = 0 Limit 1;";

        public static string GetPracticeWebPatientdue_date = "SELECT r.recallNum, rt.RecallTypeNum, r.DateDue AS due_date, r.patnum AS patient_id, rt.description AS recall_type,p.clinicnum As Clinic_Number "
                                                           + " FROM recall r "
                                                           + " Left Join patient p on p.PatNum = r.PatNum "
                                                           + " JOIN recalltype rt on r.RecallTypeNum = rt.RecallTypeNum;";

        public static string GetPracticeWebPatientWiseRecallDates = "SELECT '' AS Patient_Recall_Id,p.patnum AS Patient_EHR_id,'' AS Patient_Web_ID,r.datedue AS Recall_Date,r.DatePrevious AS Last_Recall_Date, '' AS Provider_EHR_ID,r.RecallTypeNum AS RecallType_EHR_ID,rt.description AS RecallType_Name,'' AS RecallType_Descript,"
        ...
        public static string GetPracticeWebNextApptDate = "select AptDateTime AS nextvisit_date,PatNum,AptStatus,clinicnum as Clinic_Number From appointment "
                                                       + " Where AptDateTime > @ToDate AND AptStatus NOT IN (3,5) Order by AptDateTime desc ;";

        //public static string GetPracticeWebPatientInsBenafit = " SELECT IFNULL(Sum(MonetaryAmt),0) AS TotalBenafit,IFNULL((SELECT Sum(InsPayAmt) AS UsedBenafit FROM claimproc c where patnum = @PatientId),0) AS UsedBenafit FROM benefit b "
        //                                               + " where benefitType = 5 and planNum in (SELECT PlanNum FROM inssub i where subscriber = @PatientId)and CoverageLevel = 1 ;";

        public static string GetPracticeWebPatientInsBenafit = " select patient_ehr_id,SUM(TotalBenafit) AS TotalBenafit,UsedBenafit FROM ( SELECT distinct i.subscriber AS patient_ehr_id,b.benefitNum, IFNULL((b.MonetaryAmt),0) AS TotalBenafit,c.UsedBenafit FROM benefit b inner join inssub i on i.plannum = b.plannum and benefitType = 5 and CoverageLevel = 1 "
                                                            + "  inner join ( select patnum,IFNULL(sum(InsPayAmt),0) AS UsedBenafit from claimproc group by  patnum )  c on c.patnum  = i.subscriber ) AS AA group by AA.patient_ehr_id,AA.UsedBenafit;";

        public static string GetPracticeWebPatientLastVisit_Date = "select AptDateTime AS lastvisit_date,PatNum,AptStatus,clinicnum as Clinic_Number From appointment "
                                                     + " Where AptDateTime < @ToDate  AND AptStatus in (2) Order by AptDateTime desc ;";

        public static string GetPracticeWebRecallTypeData = "SELECT RecallTypeNum AS RecallType_EHR_ID, Description AS RecallType_Name,'' AS Clinic_Number FROM recalltype;";

        public static string GetPracticeWebUserData = "select UserNum as User_EHR_ID,'' AS User_LocalDB_ID,'' as User_web_Id,UserName as First_Name,'' as Last_Name,Password as Password ,DateTLastLogin as EHR_Entry_DateTime,DateTLastLogin as Last_Updated_DateTime,curdate() as LocalDb_EntryDatetime,(case when IsHidden = 0 then 1 else 0 end) as Is_active,0 as is_deleted,0 as Is_Adit_Updated,ClinicNum as Clinic_Number,0 as Service_Install_Id from userod;";

        public static string GetPracticeWebAppointmentStatus = "SELECT DefNum AS ApptStatus_EHR_ID, ItemName AS ApptStatus_Name,  (CASE WHEN IFNULL(Category,0) = 2 THEN 'confirm' ELSE 'unshedule' END) AS ApptStatus_Type,'' As Clinic_Number  FROM definition Where Category = 13 OR Category = 2 ;";
        ...
        public static string InsertProcedureLog = "INSERT INTO Procedurelog (PatNum, AptNum, ProcDate, ProcFee, ProcStatus, ProvNum, DateEntryC, ClinicNum, CodeNum, DateTP, DateTStamp, SecDateEntry,CodeMod1,CodeMod2,CodeMod3,CodeMod4,RevCode,UnitQty,icdVersion,SecUserNumEntry)"
                                + "VALUES(@PatNum, @AptNum, @ProcDate, IFNULL((select Amount from Fee where CodeNum = @CodeNum LIMIT 1), 0), 1, @ProvNum, @DateEntryC, @ClinicNum, @CodeNum, @DateTP, @DateTStamp, @SecDateEntry,'','','','','',1,9,@EHR_User_ID);";

        public static string GetBookOperatoryAppointmenetWiseDateTime = " Select AptNum, OP, AptDateTime, CHAR_LENGTH(Pattern)*5 AS ApptMin,IFNull(o.clinicnum,a.clinicnum) as Clinic_Number,p.fname AS FirstName, p.lname AS LastName,p.WirelessPhone AS Mobile,p.Email,PP.FName AS ProviderFirstName,PP.LName AS ProviderLastName  FROM appointment a LEFT JOIN operatory o ON a.OP = o.OperatoryNum LEFT JOIn patient p on p.PatNum = a.patnum LEFT JOIN provider PP ON PP.ProvNum = a.ProvNum  "
                                                                   + " WHERE cast( a.AptDateTime  as date) = @ToDate and IFNull(o.clinicnum,a.clinicnum) = @Clinic_Number ;";

        public static string GetPracticeWebClinic = "SELECT 0 AS Clinic_Number,'head Quarters' As Description Union All SELECT ClinicNum AS Clinic_Number, Description FROM clinic ;";

        public static string Update_Status_EHR_Appointment_Live_To_PracticeWeb = " UPDATE appointment SET Confirmed = @AptStatus, DateTStamp = NOW()  "
                                       + " WHERE AptNum = @Appt_EHR_ID ";

        public static string Update_Receive_SMS_Patient_EHR_Live_To_PracticeWeb = " UPDATE patient SET TxtMsgOk = @TxtMsgOk WHERE patNum = @patient_ehr_id; ";
        ...
        public static string GetPracticeWebDocPath = " SELECT * FROM preference Where prefName = 'DocPath' ;";
        ...
        public static string GetPracticeWebPatientDiseaseData = "select d.patnum as Patient_EHR_ID,d.DiseaseDefNum as Disease_EHR_ID,DiseaseName as Disease_Name,'P' as Disease_Type,d.DateTStamp AS EHR_Entry_DateTime , "
                                                             + " ifnull(IsHidden,0) AS is_deleted,p.clinicnum Clinic_Number from disease as d left join diseasedef df on df.DiseaseDefNum = d.DiseaseDefNum left join patient p on p.patnum = d.patnum "
                                                             + " Union All "
                                                             + " select d.patnum as Patient_EHR_ID,d.allergydefNum as Disease_EHR_ID,Description as Disease_Name,'A' as Disease_Type,d.DateTStamp AS EHR_Entry_DateTime , IsHidden AS is_deleted,p.clinicnum  from allergy as d left join allergydef df on df.allergydefNum = d.allergydefNum left join patient p on p.patnum = d.patnum;";
        public static string GetPracticeWebDiseaseData = " SELECT DiseaseDefNum AS Disease_EHR_ID ,DiseaseName AS Disease_Name, "
                                                      ...
                                                      + " FROM allergydef; ";
        //public static string GetPracticeWebMedicationData = " SELECT medicationnum as Medication_EHR_ID,case when medicationnum = genericnum then medname else CONCAT(medname ,'(',(select medname from medication where medicationnum = "
        ...
        public static string GetPracticeWebMedicationData = "SELECT medicationnum as Medication_EHR_ID, "
        ...
        public static string GetPracticeWebPatientMedicationData = "select mp.patnum as Patient_EHR_ID,mp.medicationnum as Medication_EHR_ID,medicationpatnum as PatientMedication_EHR_ID,'' as Drug_Quantity,m.medname as Medication_Name,case when mp.medicationnum = genericnum then 'Drug' else 'Brand' "
                                                      + " end as Medication_Type, meddescript as Medication_Note,provnum as Provider_EHR_ID ,mp.datestart as Start_Date,mp.datestop as End_Date ,mp.DateTStamp AS EHR_Entry_DateTime , PatNote as Patient_Notes, 0 AS is_deleted,p.clinicnum Clinic_Number, "
                                                      + " case when (datestop >= curdate() or datestop = '0001-01-01') then 'True' else 'False' end as is_active from medicationpat as mp left join "
                                                      + " medication as m on m.medicationnum = mp.medicationnum left join patient p on p.patnum = mp.patnum ";


        public static string GetPracticeWebMedicalHistoryForm = " select @Service_Install_Id AS Service_Install_Id ,'' Clinic_Number,'' AS SheetDefNum_LocalDB_ID, SheetDefNum AS SheetDefNum_EHR_ID,'' AS SheetDefNum_Web_ID,Description AS Sheet_Name,FontSize,FontName,Width,Height,IsLandscape,PageCount,IsMultiPage,now() AS EHR_Entry_DateTime,now() AS Last_Sync_Date,0 AS is_deleted,0 AS Is_Adit_Updated FROM sheetdef s;";

        public static string GetPracticeWebMedicalHistoryFields_15_4 = " select @Service_Install_Id AS Service_Install_Id ,'' Clinic_Number,'' AS SheetFieldDefNum_LocalDB_ID, sfd.sheetfielddefnum AS SheetFieldDefNum_EHR_ID,'' AS SheetFieldDefNum_Web_ID,sfd.Sheetdefnum AS SheetDefNum_EHR_ID,sfd.FieldType,sfd.FieldName,sfd.fieldvalue,sfd.fontsize,sfd.FontName,sfd.Fontisbold,sfd.xpos,sfd.ypos,sfd.width,sfd.height,sfd.growthbehavior,sfd.radiobuttonvalue,sfd.RadioButtonGroup, sfd.isrequired,sfd.taborder,sfd.ReportableName,sfd.TextAlign,sfd.ItemColor,s.SheetType,now() AS EHR_Entry_DateTime,now() AS Last_Sync_Date,0 AS is_deleted, 0 AS Is_Adit_Updated FROM sheetfielddef sfd inner join sheetdef s on sfd.sheetdefnum = s.sheetdefnum WHERE sfd.fieldtype IN (1,2,6,7,8,13);";

        public static string GetPracticeWebMedicalHistoryFields_17_Plus = " select @Service_Install_Id AS Service_Install_Id ,'' Clinic_Number,'' AS SheetFieldDefNum_LocalDB_ID, sfd.sheetfielddefnum AS SheetFieldDefNum_EHR_ID,'' AS SheetFieldDefNum_Web_ID,sfd.Sheetdefnum AS SheetDefNum_EHR_ID,sfd.FieldType,sfd.FieldName,sfd.fieldvalue,sfd.fontsize,sfd.FontName,sfd.Fontisbold,sfd.xpos,sfd.ypos,sfd.width,sfd.height,sfd.growthbehavior,sfd.radiobuttonvalue,sfd.RadioButtonGroup,sfd.isrequired,sfd.taborder,sfd.ReportableName,sfd.TextAlign,sfd.ItemColor,s.SheetType,IsLocked ,TabOrderMobile,UiLabelMobile,UiLabelMobileRadioButton,now() AS EHR_Entry_DateTime ,now() AS Last_Sync_Date,0 AS is_deleted, 0 AS Is_Adit_Updated FROM sheetfielddef sfd inner join sheetdef s on sfd.sheetdefnum = s.sheetdefnum WHERE sfd.fieldtype IN (1,2,6,7,8,13);";
        ...
        public static string checkAlreadyExistsPayment = "SELECT IF( EXISTS( select 1  From payment  where paytype = @paytype and paydate = @paydate and payamt = @payamt and patnum =  @patnum and  clinicnum = @clinicnum AND paynote = @paynote ), 1, 0)";
        ...
        public static string checklogentryexist = "SELECT IF( EXISTS( select 1  From commlog  where PatNum = @PatientEHRId and commdatetime = @CommentDate and commtype = @CommTyep and  Note = @Note AND Mode_ = @Mode ), 1, 0)";
        ...
        public static string GetEHRActualVersionPracticeWeb = "select ValueString from preference where PrefName = 'ProgramVersion'";

        public static string updateProcedureLog = " Update procedurelog set AptNum = @AptNum where ProcNum = @ProcNum ";


        //rooja 
        public static string GetPracticeWebInsuranceData = "SELECT CarrierNum,CarrierName,Address,Address2,City,State,Zip,Phone,ElectID,NoSendElect,SecUserNumEntry,SecDateEntry,IsHidden,TIN,CarrierGroupName,'' as Clinic_Number from carrier; ";
    }
}
